...
async function beginWatchSession(userId, deviceId, contentKey) {
  ...
  const active = await db(`watch_sessions?user_id=eq.${encodeURIComponent(userId)}&ended_at=is.null&last_seen_at=gt.${encodeURIComponent(cutoff)}&select=id,device_id`, {prefer:""});
  if (!hasAvailableStreamSlot(active, deviceId)) {
    throw Object.assign(new Error(`This account already has ${MAX_STREAMS} active streams`), {status:409, code:"STREAM_LIMIT"});
  }
  // Starting another title on the same registered device replaces that
  // device's previous session without consuming an additional stream slot.
  await db(`watch_sessions?user_id=eq.${encodeURIComponent(userId)}&device_id=eq.${encodeURIComponent(deviceId)}&ended_at=is.null`, {
    method:"PATCH",
    body:{ended_at:new Date().toISOString()},
    prefer:"return=minimal"
