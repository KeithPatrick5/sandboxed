const crypto = require("crypto");
const {reportServerError} = require("./monitoring");

const TRIAL_HOURS = 72;
const MONTHLY_PRICE_USD = 6.99;
const MONTHLY_PRICE_CENTS = 699;
const ANNUAL_PRICE_USD = 30;
const ANNUAL_PRICE_CENTS = ANNUAL_PRICE_USD * 100;
const MEMBERSHIP_PLANS = Object.freeze({
  monthly:Object.freeze({code:"monthly", label:"Monthly", priceUsd:MONTHLY_PRICE_USD, priceCents:MONTHLY_PRICE_CENTS, interval:"month"}),
  annual:Object.freeze({code:"annual", label:"Annual", priceUsd:ANNUAL_PRICE_USD, priceCents:ANNUAL_PRICE_CENTS, interval:"year"})
});
const MAX_DEVICES = 4;
const MAX_STREAMS = 2;
const MAX_REPLACEMENTS_30_DAYS = 2;
const STREAM_TTL_SECONDS = 150;
const MAX_REQUEST_BODY_BYTES = 1024 * 1024;
const DEVICE_COOKIE_NAME = "__Host-sandboxed_device";
const rateBuckets = new Map();
let maintenancePromise = null;
let nextMaintenanceAt = 0;

function env(name, fallback = "") {
  return String(process.env[name] || fallback).trim();
}

function membershipPlan(code) {
  return MEMBERSHIP_PLANS[String(code || "").toLowerCase()] || null;
}

function stripePriceIdForPlan(code) {
  if (code === "monthly") return env("STRIPE_MONTHLY_PRICE_ID");
  if (code === "annual") return env("STRIPE_ANNUAL_PRICE_ID") || env("STRIPE_PRICE_ID");
  return "";
}

function planForStripePriceId(priceId) {
  const id = String(priceId || "");
  if (!id) return null;
  return Object.values(MEMBERSHIP_PLANS).find((plan) => stripePriceIdForPlan(plan.code) === id) || null;
}

function membershipConfigured() {
  return Boolean(
    env("SUPABASE_URL") &&
    env("SUPABASE_PUBLISHABLE_KEY") &&
    env("SUPABASE_SECRET_KEY") &&
    env("DEVICE_HASH_SECRET") &&
    env("MEMBERSHIP_ENABLED", "true") !== "false"
  );
}

function send(response, status, payload) {
  response.status(status);
  response.setHeader("Content-Type", "application/json; charset=utf-8");
  response.setHeader("Cache-Control", "no-store");
  return response.end(JSON.stringify(payload));
}

async function readStream(request) {
  const chunks = [];
  let size = 0;
  for await (const chunk of request) {
    size += chunk.length;
    if (size > MAX_REQUEST_BODY_BYTES) {
      throw Object.assign(new Error("Request body is too large"), {status:413, code:"BODY_TOO_LARGE"});
    }
    chunks.push(chunk);
  }
  return Buffer.concat(chunks);
}

async function readBody(request) {
  if (Buffer.isBuffer(request.body)) {
    try { return JSON.parse(request.body.toString("utf8")); } catch { return {}; }
  }
  if (request.body && typeof request.body === "object") return request.body;
  if (typeof request.body === "string") {
    try { return JSON.parse(request.body); } catch { return {}; }
  }
  const body = await readStream(request);
  if (!body.length) return {};
  try { return JSON.parse(body.toString("utf8")); } catch { return {}; }
}

async function readRawBody(request) {
  if (Buffer.isBuffer(request.body)) return request.body.toString("utf8");
  if (typeof request.body === "string") return request.body;
  if (request.body && typeof request.body === "object") return JSON.stringify(request.body);
  return (await readStream(request)).toString("utf8");
}

function bearerToken(request) {
  const value = String(request.headers.authorization || "");
  return value.startsWith("Bearer ") ? value.slice(7).trim() : "";
}

function clientIp(request) {
  const forwarded = String(request?.headers?.["x-forwarded-for"] || "")
    .split(",")
    .map((value) => value.trim())
    .filter(Boolean);
  // Namecheap's front proxy appends the address it received. Use the final hop
  // so a visitor cannot choose the value by sending their own X-Forwarded-For.
  return String(forwarded.at(-1) || request?.socket?.remoteAddress || "unknown").slice(0, 80);
}

function rateLimit(request, name, limit, windowSeconds) {
  const now = Date.now();
  const windowMs = Math.max(1, Number(windowSeconds)) * 1000;
  const key = `${name}:${secureHash(`rate:${clientIp(request)}`)}`;
  const current = rateBuckets.get(key);
  if (!current || current.resetAt <= now) {
    rateBuckets.set(key, {count:1, resetAt:now + windowMs});
  } else {
    current.count += 1;
    if (current.count > limit) {
      const error = new Error("Too many attempts. Please wait and try again.");
      error.status = 429;
      error.code = "RATE_LIMITED";
      error.retryAfter = Math.max(1, Math.ceil((current.resetAt - now) / 1000));
      throw error;
    }
  }
  if (rateBuckets.size > 5000) {
    for (const [bucketKey, bucket] of rateBuckets) {
      if (bucket.resetAt <= now) rateBuckets.delete(bucketKey);
    }
  }
}

function secureHash(value) {
  return crypto.createHmac("sha256", env("DEVICE_HASH_SECRET", "sandboxed-development-only"))
    .update(String(value || ""))
    .digest("hex");
}

function safeEqual(left, right) {
  const first = Buffer.from(String(left || ""));
  const second = Buffer.from(String(right || ""));
  return first.length === second.length && crypto.timingSafeEqual(first, second);
}

function signedDeviceId(deviceId) {
  const id = String(deviceId || "");
  return `${id}.${secureHash(`device-cookie:${id}`)}`;
}

function verifiedDeviceId(request) {
  const cookies = String(request?.headers?.cookie || "").split(";");
  const raw = cookies.map((part) => part.trim()).find((part) => part.startsWith(`${DEVICE_COOKIE_NAME}=`))?.slice(DEVICE_COOKIE_NAME.length + 1) || "";
  const separator = raw.lastIndexOf(".");
  if (separator < 1) return "";
  const id = raw.slice(0, separator);
  const signature = raw.slice(separator + 1);
  if (!/^[A-Za-z0-9_-]{12,160}$/.test(id)) return "";
  return safeEqual(signature, secureHash(`device-cookie:${id}`)) ? id : "";
}

function setDeviceCookie(response, deviceId) {
  response.setHeader("Set-Cookie", `${DEVICE_COOKIE_NAME}=${signedDeviceId(deviceId)}; Path=/; Max-Age=31536000; HttpOnly; Secure; SameSite=Lax`);
}

function baseUrl() {
  return env("SITE_URL", "https://sandboxed-tv.vercel.app").replace(/\/$/, "");
}

async function supabaseAuth(path, {method = "GET", token = "", body} = {}) {
  if (!membershipConfigured()) throw Object.assign(new Error("Membership is not configured"), {status:503, code:"NOT_CONFIGURED"});
  const response = await fetch(`${env("SUPABASE_URL").replace(/\/$/, "")}/auth/v1${path}`, {
    method,
    headers:{
      apikey:env("SUPABASE_PUBLISHABLE_KEY"),
      ...(token ? {Authorization:`Bearer ${token}`} : {}),
      ...(body ? {"Content-Type":"application/json"} : {})
    },
    ...(body ? {body:JSON.stringify(body)} : {}),
    signal:AbortSignal.timeout(10000)
  });
  const text = await response.text();
  let payload;
  try { payload = text ? JSON.parse(text) : {}; } catch { payload = {message:text}; }
  if (!response.ok) {
    const error = new Error(payload?.msg || payload?.message || payload?.error_description || "Authentication failed");
    error.status = response.status;
    error.code = payload?.error_code || payload?.code || "AUTH_ERROR";
    throw error;
  }
  return payload;
}

async function requireUser(request) {
  const token = bearerToken(request);
  if (!token) throw Object.assign(new Error("Sign in required"), {status:401, code:"AUTH_REQUIRED"});
  const user = await supabaseAuth("/user", {token});
  if (!user?.id) throw Object.assign(new Error("Invalid session"), {status:401, code:"INVALID_SESSION"});
  if (!user.email_confirmed_at) throw Object.assign(new Error("Verify your email before continuing"), {status:403, code:"EMAIL_NOT_VERIFIED"});
  return {user, token};
}

async function db(path, {method = "GET", body, prefer = "return=representation"} = {}) {
  if (!membershipConfigured()) throw Object.assign(new Error("Membership is not configured"), {status:503, code:"NOT_CONFIGURED"});
  const secret = env("SUPABASE_SECRET_KEY");
  const legacyServiceRole = !secret.startsWith("sb_secret_");
  const response = await fetch(`${env("SUPABASE_URL").replace(/\/$/, "")}/rest/v1/${path}`, {
    method,
    headers:{
      apikey:secret,
      ...(legacyServiceRole ? {Authorization:`Bearer ${secret}`} : {}),
      ...(body !== undefined ? {"Content-Type":"application/json"} : {}),
      ...(prefer ? {Prefer:prefer} : {})
    },
    ...(body !== undefined ? {body:JSON.stringify(body)} : {}),
    signal:AbortSignal.timeout(10000)
  });
  const text = await response.text();
  let payload;
  try { payload = text ? JSON.parse(text) : null; } catch { payload = text; }
  if (!response.ok) {
    const error = new Error(payload?.message || payload?.hint || `Database request failed (${response.status})`);
    error.status = 500;
    error.code = payload?.code || "DATABASE_ERROR";
    error.details = payload;
    throw error;
  }
  return payload;
}

async function ensureProfile(user) {
  const encoded = encodeURIComponent(user.id);
  let rows = await db(`profiles?id=eq.${encoded}&select=*`, {prefer:""});
  if (rows?.[0]) {
    scheduleMaintenance();
    return rows[0];
  }
  try {
    rows = await db("profiles", {
      method:"POST",
      body:{id:user.id, email:String(user.email || "").toLowerCase()},
      prefer:"return=representation"
    });
    scheduleMaintenance();
    return rows?.[0];
  } catch (error) {
    rows = await db(`profiles?id=eq.${encoded}&select=*`, {prefer:""});
    if (rows?.[0]) {
      scheduleMaintenance();
      return rows[0];
    }
    throw error;
  }
}

async function cleanupOldRecords(now = Date.now()) {
  const watchCutoff = new Date(now - 30 * 86400000).toISOString();
  const deviceCutoff = new Date(now - 90 * 86400000).toISOString();
  const paymentCutoff = new Date(now - 400 * 86400000).toISOString();
  await db(`watch_sessions?ended_at=not.is.null&ended_at=lt.${encodeURIComponent(watchCutoff)}`, {method:"DELETE", prefer:"return=minimal"});
  await db(`devices?revoked_at=not.is.null&revoked_at=lt.${encodeURIComponent(deviceCutoff)}`, {method:"DELETE", prefer:"return=minimal"});
  await db(`payment_events?created_at=lt.${encodeURIComponent(paymentCutoff)}`, {method:"DELETE", prefer:"return=minimal"});
  await db(`marketing_events?created_at=lt.${encodeURIComponent(paymentCutoff)}`, {method:"DELETE", prefer:"return=minimal"});
  await db(`marketing_attribution?user_id=is.null&last_seen_at=lt.${encodeURIComponent(paymentCutoff)}`, {method:"DELETE", prefer:"return=minimal"});
  return {watchCutoff, deviceCutoff, paymentCutoff};
}

function scheduleMaintenance(now = Date.now()) {
  if (!membershipConfigured() || maintenancePromise || now < nextMaintenanceAt) return maintenancePromise;
  nextMaintenanceAt = now + 3600000;
  maintenancePromise = cleanupOldRecords(now)
    .then((result) => {
      nextMaintenanceAt = Date.now() + 86400000;
      return result;
    })
    .catch((error) => {
      console.error("[sandboxed:maintenance]", {code:error?.code, message:error?.message});
      reportServerError(error, {route:"maintenance"});
      return null;
    })
    .finally(() => { maintenancePromise = null; });
  return maintenancePromise;
}

function accessState(profile) {
  const now = Date.now();
  const paidUntil = profile?.access_until ? Date.parse(profile.access_until) : 0;
  const trialEnd = profile?.trial_ends_at ? Date.parse(profile.trial_ends_at) : 0;
  if (["refunded", "disputed"].includes(String(profile?.subscription_status || "").toLowerCase())) {
    return {state:"expired", accessUntil:profile?.access_until || null, trialEndsAt:profile?.trial_ends_at || null};
  }
  if (paidUntil > now) return {state:"active", accessUntil:profile.access_until, trialEndsAt:profile.trial_ends_at};
  if (trialEnd > now) return {state:"trial", accessUntil:null, trialEndsAt:profile.trial_ends_at};
  if (profile?.trial_started_at) return {state:"expired", accessUntil:profile.access_until, trialEndsAt:profile.trial_ends_at};
  return {state:"eligible", accessUntil:profile?.access_until || null, trialEndsAt:null};
}

async function updateProfile(userId, fields) {
  const rows = await db(`profiles?id=eq.${encodeURIComponent(userId)}`, {
    method:"PATCH",
    body:{...fields, updated_at:new Date().toISOString()},
    prefer:"return=representation"
  });
  return rows?.[0];
}

function deviceInputs(body) {
  const fingerprint = String(body?.fingerprint || "").slice(0, 500);
  const fingerprintV2 = String(body?.fingerprintV2 || "").slice(0, 500);
  const deviceName = String(body?.deviceName || "Device").slice(0, 80);
  if (fingerprint.length < 12) {
    throw Object.assign(new Error("This device could not be identified"), {status:400, code:"DEVICE_ID_REQUIRED"});
  }
  return {fingerprint, fingerprintV2, deviceName};
}

async function trustedDeviceId(userId, body, request, response) {
  const existing = verifiedDeviceId(request);
  if (existing) return existing;

  // One-time migration: adopt the old browser identifier only when its hash is
  // already registered to this same account. New identities are server-made.
  const legacyId = String(body?.deviceId || "").slice(0, 160);
  if (/^[A-Za-z0-9_-]{12,160}$/.test(legacyId)) {
    const legacyHash = secureHash(`device:${legacyId}`);
    const rows = await db(`devices?user_id=eq.${encodeURIComponent(userId)}&device_key_hash=eq.${legacyHash}&select=id`, {prefer:""});
    if (rows?.length) {
      setDeviceCookie(response, legacyId);
      return legacyId;
    }
  }

  const generated = crypto.randomUUID();
  setDeviceCookie(response, generated);
  return generated;
}

async function registerDevice(userId, body, request, response) {
  const {fingerprint, fingerprintV2, deviceName} = deviceInputs(body);
  const deviceId = await trustedDeviceId(userId, body, request, response);
  const deviceKeyHash = secureHash(`device:${deviceId}`);
  const fingerprintHash = secureHash(`fingerprint:${fingerprint}`);
  const fingerprintV2Hash = fingerprintV2.length >= 12 ? secureHash(`fingerprint-v2:${fingerprintV2}`) : "";
  const ipHash = secureHash(`ip:${clientIp(request)}`);
  try {
    const rows = await db("rpc/register_device_atomic", {
      method:"POST",
      body:{
        p_user_id:userId,
        p_device_key_hash:deviceKeyHash,
        p_fingerprint_hash:fingerprintHash,
        p_fingerprint_v2_hash:fingerprintV2Hash || null,
        p_name:deviceName,
        p_last_ip_hash:ipHash,
        p_max_devices:MAX_DEVICES
      },
      prefer:""
    });
    if (!rows?.[0]) throw Object.assign(new Error("Device could not be registered"), {status:500, code:"DEVICE_REGISTRATION_FAILED"});
    return {...rows[0], fingerprintHash, fingerprintV2Hash, ipHash};
  } catch (error) {
    if (error?.message === "DEVICE_LIMIT") {
      throw Object.assign(new Error(`Your account already has ${MAX_DEVICES} active devices`), {status:409, code:"DEVICE_LIMIT"});
    }
    throw error;
  }
}

async function listDevices(userId) {
  return db(`devices?user_id=eq.${encodeURIComponent(userId)}&revoked_at=is.null&select=id,name,created_at,last_seen_at&order=last_seen_at.desc`, {prefer:""});
}

async function revokeDevice(userId, deviceId) {
  try {
    await db("rpc/revoke_device_atomic", {
      method:"POST",
      body:{p_user_id:userId, p_device_id:deviceId, p_max_replacements:MAX_REPLACEMENTS_30_DAYS},
      prefer:""
    });
  } catch (error) {
    if (error?.message === "REPLACEMENT_LIMIT") {
      throw Object.assign(new Error("Two device replacements have already been used in the last 30 days"), {status:429, code:"REPLACEMENT_LIMIT"});
    }
    if (error?.message === "DEVICE_NOT_FOUND") {
      throw Object.assign(new Error("Device not found"), {status:404, code:"DEVICE_NOT_FOUND"});
    }
    throw error;
  }
}

async function trialEligibility(profile, device) {
  if (profile.trial_started_at) {
    return {eligible:false, code:"TRIAL_USED", message:"This account has already used its free trial."};
  }
  const sameFingerprint = await db(`trial_claims?fingerprint_hash=eq.${device.fingerprintHash}&select=user_id`, {prefer:""});
  if ((sameFingerprint || []).some((claim) => claim.user_id !== profile.id)) {
    return {eligible:false, code:"TRIAL_USED", message:"The free trial has already been used on this device."};
  }
  if (device.fingerprintV2Hash) {
    const sameFingerprintV2 = await db(`trial_claims?fingerprint_v2_hash=eq.${device.fingerprintV2Hash}&select=user_id`, {prefer:""});
    if ((sameFingerprintV2 || []).some((claim) => claim.user_id !== profile.id)) {
      return {eligible:false, code:"TRIAL_USED", message:"The free trial has already been used on this device."};
    }
  }
  const sameIp = await db(`trial_claims?ip_hash=eq.${device.ipHash}&select=id`, {prefer:""});
  if ((sameIp || []).length >= 3) {
    return {eligible:false, code:"TRIAL_USED", message:"The free-trial limit has been reached on this network."};
  }
  return {eligible:true, code:null, message:""};
}

async function startTrial(profile, device) {
  if (profile.trial_started_at) return profile;
  const eligibility = await trialEligibility(profile, device);
  if (!eligibility.eligible) {
    throw Object.assign(new Error(eligibility.message), {status:402, code:eligibility.code});
  }
  const started = new Date();
  const ends = new Date(started.getTime() + TRIAL_HOURS * 3600000);
  try {
    await db("trial_claims", {
      method:"POST",
      body:{
        user_id:profile.id,
        fingerprint_hash:device.fingerprintHash,
        ...(device.fingerprintV2Hash ? {fingerprint_v2_hash:device.fingerprintV2Hash} : {}),
        ip_hash:device.ipHash
      },
      prefer:"return=minimal"
    });
  } catch (error) {
    if (error.code === "23505") throw Object.assign(new Error("The free trial has already been used on this device"), {status:402, code:"TRIAL_USED"});
    throw error;
  }
  return updateProfile(profile.id, {trial_started_at:started.toISOString(), trial_ends_at:ends.toISOString()});
}

async function beginWatchSession(userId, deviceId, contentKey) {
  try {
    const rows = await db("rpc/begin_watch_session_atomic", {
      method:"POST",
      body:{
        p_user_id:userId,
        p_device_id:deviceId,
        p_content_key:String(contentKey).slice(0,120),
        p_stream_ttl_seconds:STREAM_TTL_SECONDS,
        p_max_streams:MAX_STREAMS
      },
      prefer:""
    });
    if (!rows?.[0]) throw Object.assign(new Error("Playback session could not be started"), {status:500, code:"SESSION_START_FAILED"});
    return rows[0];
  } catch (error) {
    if (error?.message === "STREAM_LIMIT") {
      throw Object.assign(new Error(`This account already has ${MAX_STREAMS} active streams`), {status:409, code:"STREAM_LIMIT"});
    }
    throw error;
  }
}

function activeStreamDeviceIds(rows, excludedDeviceId = "") {
  return new Set((rows || [])
    .map((row) => String(row?.device_id || ""))
    .filter((deviceId) => deviceId && deviceId !== String(excludedDeviceId || "")));
}

function hasAvailableStreamSlot(rows, deviceId, maxStreams = MAX_STREAMS) {
  return activeStreamDeviceIds(rows, deviceId).size < maxStreams;
}

async function touchWatchSession(userId, sessionId, end = false) {
  const sessionPath = `watch_sessions?id=eq.${encodeURIComponent(sessionId)}&user_id=eq.${encodeURIComponent(userId)}&ended_at=is.null`;
  if (end) {
    const rows = await db(sessionPath, {
      method:"PATCH",
      body:{ended_at:new Date().toISOString()},
      prefer:"return=representation"
    });
    if (!rows?.length) throw Object.assign(new Error("Playback session expired"), {status:404, code:"SESSION_EXPIRED"});
    return rows[0];
  }

  const sessions = await db(`${sessionPath}&select=id,device_id`, {prefer:""});
  const watch = sessions?.[0];
  if (!watch) throw Object.assign(new Error("Playback session expired"), {status:404, code:"SESSION_EXPIRED"});

  const [profiles, devices] = await Promise.all([
    db(`profiles?id=eq.${encodeURIComponent(userId)}&select=id,trial_started_at,trial_ends_at,access_until`, {prefer:""}),
    db(`devices?id=eq.${encodeURIComponent(watch.device_id)}&user_id=eq.${encodeURIComponent(userId)}&revoked_at=is.null&select=id`, {prefer:""})
  ]);

  try {
    validatePlaybackAuthorization(profiles?.[0], devices?.[0]);
  } catch (error) {
    await db(sessionPath, {
      method:"PATCH",
      body:{ended_at:new Date().toISOString()},
      prefer:"return=minimal"
    }).catch(() => {});
    throw error;
  }

  const rows = await db(sessionPath, {
    method:"PATCH",
    body:{last_seen_at:new Date().toISOString()},
    prefer:"return=representation"
  });
  if (!rows?.length) throw Object.assign(new Error("Playback session expired"), {status:404, code:"SESSION_EXPIRED"});
  return rows[0];
}

function validatePlaybackAuthorization(profile, device) {
  const access = accessState(profile);
  if (access.state !== "trial" && access.state !== "active") {
    throw Object.assign(new Error("Your playback access has ended. Choose a membership to continue."), {status:402, code:"PLAYBACK_ACCESS_ENDED"});
  }
  if (!device?.id) {
    throw Object.assign(new Error("This device is no longer authorized for playback."), {status:403, code:"DEVICE_REVOKED"});
  }
  return access;
}

async function extendAccess(userId, until, fields = {}) {
  const profileRows = await db(`profiles?id=eq.${encodeURIComponent(userId)}&select=*`, {prefer:""});
  if (!profileRows?.[0]) throw Object.assign(new Error("Membership profile not found"), {status:404, code:"PROFILE_NOT_FOUND"});
  const current = profileRows[0].access_until ? Date.parse(profileRows[0].access_until) : 0;
  const requested = until instanceof Date ? until.getTime() : Date.parse(until);
  const accessUntil = new Date(Math.max(current, requested || 0)).toISOString();
  return updateProfile(userId, {...fields, access_until:accessUntil});
}

async function suspendMembershipAccess(userId, status) {
  const profile = await updateProfile(userId, {subscription_status:String(status || "suspended")});
  await db(`watch_sessions?user_id=eq.${encodeURIComponent(userId)}&ended_at=is.null`, {
    method:"PATCH",
    body:{ended_at:new Date().toISOString()},
    prefer:"return=minimal"
  });
  return profile;
}

function handlerError(response, error) {
  console.error("[sandboxed]", {code:error?.code, message:error?.message, details:error?.details});
  if (error?.retryAfter) response.setHeader("Retry-After", String(error.retryAfter));
  const status = Number(error?.status) || 500;
  if (status >= 500) reportServerError(error, {route:response?.req?.url?.split("?")[0] || "api"});
  return send(response, status, status >= 500
    ? {error:"The service is temporarily unavailable.", code:"SERVER_ERROR"}
    : {error:error?.message || "Request failed", code:error?.code || "REQUEST_ERROR"});
}

module.exports = {
  TRIAL_HOURS,
  MONTHLY_PRICE_USD,
  MONTHLY_PRICE_CENTS,
  ANNUAL_PRICE_USD,
  ANNUAL_PRICE_CENTS,
  MEMBERSHIP_PLANS,
  MAX_DEVICES,
  MAX_STREAMS,
  MAX_REPLACEMENTS_30_DAYS,
  env,
  membershipPlan,
  stripePriceIdForPlan,
  planForStripePriceId,
  baseUrl,
  membershipConfigured,
  send,
  readBody,
  readRawBody,
  bearerToken,
  clientIp,
  rateLimit,
  secureHash,
  safeEqual,
  signedDeviceId,
  verifiedDeviceId,
  supabaseAuth,
  requireUser,
  db,
  ensureProfile,
  cleanupOldRecords,
  scheduleMaintenance,
  accessState,
  updateProfile,
  registerDevice,
  listDevices,
  revokeDevice,
  trialEligibility,
  startTrial,
  activeStreamDeviceIds,
  hasAvailableStreamSlot,
  beginWatchSession,
  validatePlaybackAuthorization,
  touchWatchSession,
  extendAccess,
  suspendMembershipAccess,
  handlerError
};
